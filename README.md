...
User Query
    |
    v
Supervisor Agent (AgentCore Runtime, Strands, Claude Haiku 4.5)
    |
    | routes via tool-calling
    |
    +-------------------+-------------------+
    v                   v                   v
Technical Agent     Billing Agent      Refunds Agent   (AgentCore Runtime, Strands)
    |                   |                   |
    v                   v                   v
Technical KB        Billing KB         Refunds KB      (Bedrock Knowledge Bases)
    |                   |                   |
    +-------------------+-------------------+
                        |
                        v
...
Each specialist agent has its own Knowledge Base, so retrieval stays domain-scoped instead of mixing billing content into a technical query (or vice versa). The supervisor never answers directly — it only decides which specialist(s) to consult and synthesizes their responses, including handling questions that span multiple domains in a single query.
...
- **Domain-scoped retrieval**: separating billing/technical/refunds into distinct Knowledge Bases keeps each specialist's context clean and reduces irrelevant retrieval.
...
- **ARN sub-resource scoping**: IAM permission checks for `InvokeAgentRuntime` are evaluated against the `/runtime-endpoint/DEFAULT` sub-resource, not the bare Runtime ARN — required both forms in the policy.
- **SSE response parsing**: AgentCore Runtime responses stream as Server-Sent Events (`data: {json}` per line), not plain newline-delimited JSON — required adjusting the supervisor's response-parsing logic after inspecting raw output via curl.
